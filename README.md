...
以原有登入、地圖、資料工作台、風險規則與稽核功能為基礎，點選導覽列「官方資料工作區」，或開啟 `preview.html?dataset=official`，載入新北市官方資料。原網址與原有示範流程維持相容。

本地啟動：`python3 scripts/run_local.py --port 8090`，開啟 `http://127.0.0.1:8090/preview.html`。啟動器使用本地 SQLite。

官方快照含 1,122 筆整合紀錄、3,320 筆歷史與 3,244 份報告。立案字號、月費、實際在園人數等缺值不推估；官方來源未提供座標，地圖保留行政區縮放，園所可透過清單查詢。原有示範地圖標記仍保留。

官方模式沿用 API_BASE 支援 S3，工作紀錄使用 `ntpc_official` 儲存鍵與獨立瀏覽器快取；RDS 初始化會將 reviews/cases 的園所識別碼欄位遷移為 BIGINT。CI 會在隔離的 PostgreSQL schema 驗證遷移及工作區隔離。詳見 [資料管線說明](docs/DATA_PIPELINE.md)。
