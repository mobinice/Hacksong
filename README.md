...
開啟 http://127.0.0.1:8090/preview.html。此啟動器固定使用本地 SQLite，僅監聽 127.0.0.1；不連線 AWS RDS。

預設園所資料工作台、風險總覽與稽核待覆核清單共用官方 JSON 快照，支援分頁、搜尋、行政區篩選與已下載評鑑報告。人工覆核後建立稽核案件；缺少裁罰、財務及輿情輸入時標示資料不足，不補入示範數值。

本次整合 1,122 筆紀錄、3,320 筆評鑑歷史、3,244 份報告，涵蓋全部 29 個行政區。立案字號、每生月費、實際在園人數未公開者保持空值。完整說明見 [資料管線說明](docs/DATA_PIPELINE.md)。

本地驗證：`python3 -m unittest discover -s tests -v`。
