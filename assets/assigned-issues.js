...
(() => {
  ...
  const localSemanticMapping=()=>{
    ...
    for(const field of importFields()){
      ...
      if(best.index>=0&&numericImportFields.includes(field[0])){
        const values=st.rows.map(r=>String(r[best.index]??'').trim()).filter(Boolean);
        if(values.length&&values.filter(v=>!Number.isFinite(Number(v.replace(/[,，元\s]/g,'')))).length/values.length>0.2){best.confidence=Math.min(best.confidence,25);best.reason='欄位不符：內容不是此欄位需要的數字';}
      }
      result[field[0]]=best;
    }
    return result;
  ...
  async function applySemanticMapping(){
    ...
    for(const field of importFields()){
      ...
      if(!item.reason.startsWith('欄位不符')&&serverHit&&serverHit.confidence>item.confidence){item.confidence=serverHit.confidence;item.reason=serverHit.reason;}
      if(item.index>=0&&item.confidence>=52&&!used.has(item.index)){st.mapping[key]=item.index;st.selected[key]=true;used.add(item.index);}else {st.mapping[key]=-1;st.selected[key]=key==='name';}
    }
  ...
  mappingPanel=()=>{
    ...
    const review=fields.filter(f=>st.mapping[f[0]]<0||((st.aiMapping?.[f[0]]?.confidence||0)<75)).length;
    const targetPanel=st.categories.includes('basic')?'':`<div class="notice"><b>指定本次資料所屬園所</b><p>本檔案的所有資料列會匯入同一園所；多園所檔案請返回並勾選「園所基本資料」。</p><label class="field">既有園所<select aria-label="匯入目標園所" onchange="importSession.targetSchool=this.value"><option value="">請選擇園所</option>${schools.map(s=>`<option value="${esc(s.id)}" ${String(st.targetSchool)===String(s.id)?'selected':''}>${esc(s.name)}（${esc(s.district)}）</option>`).join('')}</select></label><label class="field">本次資料年度<input aria-label="匯入目標年度" type="number" min="1900" max="2100" value="${esc(st.targetYear||'')}" oninput="importSession.targetYear=this.value"></label></div>`;
    return `<div class="panel"><div class="panel-head"><div><h2>AI 欄位對應，請人工確認</h2><p>${esc(st.file)} · ${st.rows.length} 筆 · 已對應 ${recognized} 個欄位</p></div><button onclick="importSession.step=1;renderImport()">重新選擇檔案</button></div><div class="pad"><label class="field">工作表／資料表<select aria-label="工作表" onchange="switchImportSheet(this.value)">${st.sheets.map((s,i)=>`<option value="${i}" ${st.sheetIndex===i?'selected':''}>${esc(s.name)}</option>`).join('')}</select></label>${targetPanel}<div class="ai-banner" role="status"><div><strong>${st.aiBusy?'Demo AI 正在比對…':'已完成可解釋的語意比對'}</strong><p>${review?`欄位不符或低信心：${review} 個項目未可靠對應，請確認欄位、檔案或所選類別。`:'目前建議皆可進一步確認；仍不會自動匯入。'}</p></div><span class="pill">${esc(st.aiProvider||'Demo AI')}</span></div><div id="mapping-error" class="form-error" role="alert"></div></div><div class="table-wrap"><table class="mapping-table"><thead><tr><th>匯入</th><th>平台欄位／AI 理由</th><th>檔案欄位</th><th>內容範例</th></tr></thead><tbody>${fields.map(f=>{const a=st.aiMapping?.[f[0]],confidence=a?.confidence||0,kind=confidence>=75?'':confidence?'review':'none';return `<tr><td><input type="checkbox" aria-label="匯入${esc(f[1])}" ${st.selected[f[0]]?'checked':''} ${f[0]==='name'?'disabled':''} onchange="importSession.selected['${f[0]}']=this.checked"></td><td><b>${esc(f[1])}${f[0]==='name'?' *':''}</b><small>${esc(a?.reason||'尚未執行語意比對')}</small><span class="confidence ${kind}">${a?.manual?'人工指定':confidence?`AI 信心 ${confidence}%`:'AI 信心 0% · 欄位不符'}</span></td><td><select aria-label="對應${esc(f[1])}" onchange="Y.mappingChanged('${f[0]}',this.value)"><option value="-1">請選擇檔案欄位</option>${st.headers.map((h,i)=>`<option value="${i}" ${st.mapping[f[0]]===i?'selected':''}>${esc(h)}</option>`).join('')}</select></td><td><small id="example-${f[0]}">${mappingExample(f[0])}</small></td></tr>`}).join('')}</tbody></table></div><div class="import-actions"><small>AI 只提出建議；未對應的欄位不匯入，低信心欄位請人工確認。</small><button class="primary" onclick="buildImportPreview()">下一步：預覽整理結果 →</button></div></div>`;
  };
