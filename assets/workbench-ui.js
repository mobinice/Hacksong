...
(() => {
  Object.assign(functionHelp, {
    ...
    rules: {title:'風險判斷規則',intro:'設定異常的判斷条件、風險構面及權重，讓分數可以解釋與追溯。',steps:['編輯規則門檻、構面與權重，或調整啟用狀態。','點「調整權重」一起修改規則比例；全部規則合計 100%，停用規則不計分。','儲存後重新計算分數，詳情的關注原因可查看公式與來源。'],note:'裁罰按次累加：每次為該規則權重的一半，上限為該規則權重。人工回填另列加減分；缺資料不當成 0。權重為示範設定，非官方評鑑標準。'},
