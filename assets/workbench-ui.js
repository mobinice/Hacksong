...
(() => {
  Object.assign(functionHelp, {
    fields: {title:'欄位管理',intro:'定義平台要整理的資料，以及園所詳情要顯示哪些欄位。',steps:['新增或編輯欄位名稱、類型、單位與預期來源。','查看欄位被哪些風險規則使用。','設定是否在園所詳情顯示；工作台可另勾選顯示與匯出欄位。'],note:'新增欄位不會自動計分，也不會自動抓取資料。每生人事成本由年度人事費與實際學生數計算。'},
    rules: {title:'風險判斷規則',intro:'設定異常的判斷条件、風險構面及權重，讓分數可以解釋與追溯。',steps:['編輯規則門檻、構面與權重，或調整啟用狀態。','啟用規則的權重合計必須為 100% 才能發布。','發布後重新計算分數，詳情的「分數詳情」可查看公式與來源。'],note:'裁罰按次累加：權重 10% 時，每次加 10 分。總分可能超過 100；缺資料不當成 0。權重為示範設定，非官方評鑑標準。'},
