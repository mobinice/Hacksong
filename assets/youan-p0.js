...
(() => {
  ...
  function recalc(reason,remember=false){
    ...
    for(const s of schools){
      ...
      for(const impact of db.externalImpacts?.[s.id]||[]){const ocr=String(impact.id).includes('ocr');rows.push({
        ruleId:impact.id||'external',name:impact.name||'人工查核回填',dimension:impact.dimension||dims[0],
        weight:0,threshold:0,enabled:true,raw:Number(impact.delta||0),contribution:ocr?0:Number(impact.delta||0),
        hit:!ocr&&Number(impact.delta||0)>0,formula:ocr?'OCR 待覆核證據；權重 0%，實際加分 0 分（不直接加總辨識結果）':impact.formula||`外部證據調整 ${Number(impact.delta||0)} 分`,
        observed:impact.observed||impact.formula||'承辦人已完成證據確認',thresholdStr:ocr?'未設定計分權重':'人工查核回填',
        action:impact.action||'由承辦人確認證據與後續處置',keys:[],missing:[]
      });}
      const dimScores = {
        '法遵／裁罰／評鑑': 0,
        '財務／收費': 0,
    ...
    if(remember){
      model.history.unshift({at,reason,version:`1.${db.version}`,year:model.year,assessments:Object.fromEntries(Object.entries(before).map(([id,a])=>[id,{score:a.score,coverage:a.coverage,version:a.version}]))});
      model.history=model.history.slice(0,20);
    }
  }
  recalc('載入');
  persist();
  Y.registerExternalSchools=rows=>{
    for(const s of rows){
      const existing=schools.find(x=>String(x.id)===String(s.id));
      if(existing)Object.assign(existing,s);else schools.push(s);
      if(!districts.includes(s.district))districts.push(s.district);
      // Official identity/evaluations remain separate from explicitly labelled demo finance.
      const n=Number(String(s.id).replace(/\D/g,''))||1, count=Math.max(1,Math.round(s.capacity*.8));
      const evalResult=s.evaluations[0]?.result;
      const official={capacity:s.capacity,...(evalResult?{evaluationResult:/部分|追蹤|待改善|不符合/.test(evalResult)?'待改善':'符合'}:{})};
      const demo={studentCount:count,previousStudentCount:Math.max(1,Math.round(count/1.05)),penaltyCount:n%7===0?2:0,tuition:5000,staffCost:count*(n%3===0?146000:80000),previousStaffCost:Math.round(count*(n%3===0?146000:80000)/(n%3===0?1.4:1.03)),revenue:count*(n%4===0?92000:60000),signalCount:n%5===0?10:2,signalBaseline:2};
      for(const [key,value] of Object.entries({...demo,...official})){
        if(s.complete<60&&!['capacity','evaluationResult','penaltyCount'].includes(key))continue;
        if(records(s.id,'2025')[key])continue;
        const publicField=key in official;
        addObservation(s.id,'2025',key,value,publicField?'教育部公開園所資料':'合成示範財務／事件資料（非本園實際數據）',publicField?'API 園所欄位':'Demo 情境資料',`${s.name}｜${label(key)}：${value}｜${publicField?'公開欄位；評鑑年度另列於評鑑紀錄':'合成示範，非官方或實際營運數據'}`);
      }
      s.demoRisk=true;
    }
    recalc('同步園所與示範試算資料');persist();
  };
  Y.hydrate=()=>{model=db.p0||={data:{},year:'2025',assessments:{},history:[],revision:0};seed();const external=schools.filter(s=>s.external);if(external.length)Y.registerExternalSchools(external);else recalc('從資料庫載入');};
  const assessment=s=>model.assessments[s.id];
  Y.recalculate=(reason='外部證據更新')=>{recalc(reason,true);persist();render();};
  Y.assessment=id=>model.assessments[id];
  ...
  function sourcesHTML(id,key){
    ...
    return `${r.conflict?'<div class="import-alert">同年度來源值不同，暫不參與計算。請檢查口徑，選擇採用的來源。</div>':''}${r.candidates.map(c=>`<div class="source-card ${c.id===r.chosen?'active':''}"><b>${esc(c.source)}</b><p>${esc(c.locator)} · 資料年度 ${esc(model.year)}</p><small>取得時間 ${esc(c.at)}</small><pre>${esc(c.excerpt)}</pre><p>${esc(label(key))}：<b>${esc(display(c.value))}</b></p>${r.conflict||c.id!==r.chosen?`<button onclick="Y.resolve(${esc(JSON.stringify(id))},'${key}','${c.id}')">確認採用此來源</button>`:'<span class="data-status">目前採用</span>'}</div>`).join('')}`;
  }
  ...
  Y.evidence=(id,rid)=>{
    ...
    openModal('判斷依據與數值比對', `
      ...
      <div style="margin-bottom:18px">
        ...
        <span class="pill ${r.contribution > 0 ? 'high' : 'low'}" style="margin-left:6px">${r.contribution > 0 ? `觸發加權 ${r.contribution>=0?'+':''}${r.contribution} 分` : '未觸發 (0 分)'}</span>
      </div>
  ...
  detail=()=>{
    ...
    const scoreCard = `<div class="panel pad">
      ...
      <div class="dimension-grid">
        ${dims.map(d=>{
          const rr=a.rows.filter(r=>r.dimension===d&&r.enabled),
    ...
    const reasonsPanel = `<div class="panel">
      ...
      ${a.rows.filter(r=>r.hit).map((r,i)=>`<div class="reason risk-reason">
        ...
        <div>
          <h3>${esc(r.name)} <span class="risk-points">${r.contribution>=0?'+':''}${r.contribution} 分</span></h3>
          ${tags(r)}
          <p>${esc(r.formula)}</p>
          <button class="link" onclick="Y.evidence(${esc(JSON.stringify(s.id))},'${r.ruleId}')">查看判斷依據與數值比對 ↗</button>
        </div>
      ...
      ${a.rows.filter(r=>r.raw===null).map(r=>`<div class="reason">
        <div>
          ...
          ${tags(r)}
          <p>${esc(r.formula)}</p>
          <button class="link" onclick="Y.evidence(${esc(JSON.stringify(s.id))},'${r.ruleId}')">查看缺漏／衝突來源</button>
        </div>
      </div>`).join('')}
    </div>`;
  ...
  rulesPanel=()=>{
    return `<div class="panel">
      ...
      <div class="pad">
        ...
        <p class="data-hint">各構面占比由規則權重動態加總。開關切換或修改門檻時，評分引擎即時重算各園所總分與 4 構面分布，並同步保存至後端資料庫。</p>
      </div>
    </div>
    <div class="panel">
      <div class="panel-head">
  ...
  Y.editWeights=focus=>{
    openModal('調整風險規則權重',`<form onsubmit="Y.saveWeights(event)"><p>調整各事件占總分的比例；全部規則合計需為 100%。停用規則不計分。</p><div class="form-grid" style="margin-top:18px">${db.rules.map((r,i)=>`<label class="field">${esc(r.name)}${r.enabled?'':'（已停用）'}<input aria-label="${esc(r.name)}權重" name="weight${i}" type="number" min="0" max="100" step="1" required value="${r.weight}" oninput="Y.weightTotal(this.form)"></label>`).join('')}</div><div class="notice" style="margin-top:18px">合計：<b id="weight-draft-total">${db.rules.reduce((n,r)=>n+Number(r.weight),0)}%</b> ／ 100%</div><p id="weights-error" class="form-error" role="alert"></p><div class="form-actions"><button type="button" onclick="closeModal()">取消</button><button class="primary">儲存並重算所有園所</button></div></form>`);
    if(Number.isInteger(focus))document.querySelector(`[name="weight${focus}"]`)?.focus();
  };
  Y.weightTotal=form=>{document.getElementById('weight-draft-total').textContent=db.rules.reduce((n,r,i)=>n+Number(form.elements['weight'+i].value||0),0)+'%';};
  Y.saveWeights=event=>{
    event.preventDefault();const form=event.target,weights=db.rules.map((r,i)=>Number(form.elements['weight'+i].value));
    if(weights.some(w=>!Number.isFinite(w)||w<0||w>100)||round(weights.reduce((a,b)=>a+b,0))!==100){document.getElementById('weights-error').textContent='權重合計必須為 100%，請調整後再儲存。';return;}
    const previous=clone(db.rules);db.rules.forEach((r,i)=>r.weight=weights[i]);db.version++;
    db.history.unshift({version:db.version,by:db.profile?.name||'承辦人',at:stamp(),date:isoToday(),text:db.rules.map((r,i)=>`${r.name}：${previous[i].weight}% → ${r.weight}%`).join('；')});
    recalc('調整規則權重',true);
    if(!persist()){db.rules=previous;db.version--;db.history.shift();recalc('還原');document.getElementById('weights-error').textContent='儲存失敗，已還原權重。';return;}
    closeModal();render();toast('已調整權重，總覽、詳情與資料工作台分數已同步重算');
  };
  toggleRule=i=>{
  ...
  editRule=i=>{
    ...
    openModal('編輯風險判斷規則',`<form onsubmit="submitRule(event,${i})">
      ...
      <div class="notice" style="margin-top:14px">儲存後會動態重新計算各園所分數與 4 構面分布，並同步保存；重新整理後仍會保留。</div>
      <div class="form-actions">
  ...
  settings=()=>{previousSettings();document.querySelectorAll('.tabs button').forEach(b=>{if(b.textContent==='資料欄位')b.textContent='欄位管理';});const note=$('#page>.note');if(note)note.textContent='資料會同步保存於瀏覽器與後端資料庫。規則與門檻變更即時動態重算；若需清空可使用「重設為初始示範資料」。';};
  ...
  function selectedImportExample(){
    if(!importSession.categories.length){importSession.message='請先選擇至少一個資料類別，再使用範例。';renderImport();return null;}
    const school=schoolById(0),values={schoolId:caseNumber(0),name:school.name,district:school.district,address:school.address,type:school.type,year:'2025',capacity:120,studentCount:100,sourceStudentCount:100,previousStudentCount:95,penaltyCount:0,staffCost:8000000,previousStaffCost:7900000,tuition:5000,revenue:6000000,signalCount:2,signalBaseline:2,extendedFee:1000,otherFee:500,mealCost:600000,penaltyDate:'2025-03-18',penaltyReason:'合成示範：無裁罰',evaluationDate:'2025-03-18',evaluationResult:'符合',improvement:'合成示範：無待改善事項',signalPeriod:'2025-09-01～2025-09-07',signalTopic:'合成示範：一般討論'};
    const fields=importFields(),headers=fields.map(f=>f[1]),row=fields.map(f=>values[f[0]]??(['數字','金額'].includes(db.fields.find(x=>x.key===f[0])?.type)?0:'合成示範'));
    return {headers,row,name:importSession.categories.map(k=>importCatalog[k].label).join('_')+'_合成範例'};
  }
  const csvEncode=rows=>rows.map(r=>r.map(v=>'"'+String(v??'').replace(/"/g,'""')+'"').join(',')).join('\r\n');
  Y.downloadExample=()=>{const x=selectedImportExample();if(x)downloadBlob(new Blob(['\ufeff'+csvEncode([x.headers,x.row])],{type:'text/csv;charset=utf-8'}),x.name+'.csv');};
  Y.downloadText=()=>{const x=selectedImportExample();if(x)downloadBlob(new Blob([x.headers.map((h,i)=>h+'：'+x.row[i]).join('\n')],{type:'text/plain;charset=utf-8'}),x.name+'.txt');};
  const chooseCategory=chooseImportCategory;
  chooseImportCategory=(key,on)=>{chooseCategory(key,on);importSession.message='';renderImport();};
  uploadPanel=()=>`<div class="panel"><div class="panel-head"><div><h2>匯入並整合園所資料</h2><p>真實讀取本機檔案，人工確認後才更新；未串接 AI 或雲端。</p></div><button onclick="Y.demoImport()">載入所選類別範例</button></div><div class="pad"><div class="category-grid">${Object.entries(importCatalog).map(([k,v])=>`<label class="category-option"><input type="checkbox" ${importSession.categories.includes(k)?'checked':''} onchange="chooseImportCategory('${k}',this.checked)"><span><strong>${esc(v.label)}</strong><small>${esc(v.hint)}</small></span></label>`).join('')}</div><div class="notice" style="margin-top:16px">本次類別：${esc(importSession.categories.map(k=>importCatalog[k].label).join('、')||'尚未選擇')}。範例與欄位對應只包含所選類別；歸戶欄位統一放在「園所基本資料」。只匯入其他類別時，請在下一步指定既有園所與年度；多園所批次匯入請一併勾選園所基本資料。</div>${db.fields.some(f=>f.key?.startsWith('custom_'))?`<label class="check"><input type="checkbox" ${importSession.includeCustom?'checked':''} onchange="importSession.includeCustom=this.checked;renderImport()">另加入自訂欄位</label>`:''}<div class="upload-box" style="margin-top:18px"><label for="import-file"><b>選擇檔案（最多 10 MB／1,000 筆）</b></label><input id="import-file" type="file" accept=".csv,.xlsx,.json,.txt,.pdf,.docx" onchange="recognizeFile(this.files[0])" ${importSession.busy?'disabled':''}><small>CSV、Excel、JSON；文字 PDF／Word／TXT 支援一行一欄的固定格式。掃描 PDF 尚不支援。</small><label>CSV／TXT 編碼 <select onchange="importSession.encoding=this.value">${options(['utf-8','big5'],importSession.encoding)}</select></label><div class="toolbar"><button onclick="Y.downloadExample()">下載 CSV 範例</button><button onclick="Y.downloadText()">下載文件格式範例</button></div></div>${importSession.busy?'<div class="loading"><span class="spinner"></span><p>讀取並比對欄位…</p></div>':''}${importSession.message?`<div class="import-alert" role="alert" style="margin-top:15px">${esc(importSession.message)}</div>`:''}<div class="notice" style="margin-top:18px">目前只在瀏覽器本機處理，來源摘錄會保存於本機。請使用合規或合成資料；本機功能完成不代表允許將財務或個資上傳競賽 AWS。</div></div></div>`;
  Y.demoImport=()=>{const x=selectedImportExample();if(!x)return;importSession.file=x.name+'.csv';importSession.sheets=[{name:x.name,rows:[x.headers,x.row],locations:['第 1 列','第 2 列']}];loadImportSheet(0);importSession.step=2;renderImport();};
  ...
  importFields=()=>{const fields=[];for(const cat of importSession.categories)for(const f of importCatalog[cat].fields)if((cat==='basic'||!['schoolId','name','district','year'].includes(f[0]))&&!fields.some(x=>x[0]===f[0]))fields.push(f);for(const f of db.fields.filter(f=>importSession.includeCustom&&f.key?.startsWith('custom_')))fields.push([f.key,f.name,[f.name,f.key]]);return fields;};
  ...
  const baseBuildPreview=buildImportPreview;
  buildImportPreview=()=>{
    const st=importSession;
    if(!importFields().some(f=>st.selected[f[0]]&&st.mapping[f[0]]>=0)){$('#mapping-error').textContent='欄位不符：未找到可匯入的所選類別欄位，請更換檔案或調整資料類別。';return;}
    if(!st.categories.includes('basic')){
      const target=schoolById(st.targetSchool);
      if(!target||!/^\d{4}$/.test(st.targetYear||'')||+st.targetYear<1900||+st.targetYear>2100){$('#mapping-error').textContent='請先指定既有園所與有效資料年度；多園所檔案請一併選擇園所基本資料。';return;}
      baseBuildPreview();
      if(st.step===3){for(const row of st.preview)Object.assign(row.values,{schoolId:caseNumber(target.id),name:target.name,district:target.district,year:st.targetYear});renderImport();}
    }else baseBuildPreview();
  };
  ...
  fieldsPanel=()=>`<div class="panel"><div class="panel-head"><div><h2>欄位管理</h2><p>設定匯入欄位與資料定義；計分用途由規則自動列出。</p></div><button class="primary" onclick="editField()">＋ 新增資料欄位</button></div><div class="table-wrap"><table><thead><tr><th>欄位名稱</th><th>類型／單位</th><th>預期資料來源</th><th>被哪些規則使用</th><th>操作</th></tr></thead><tbody>${db.fields.map((f,i)=>{const used=db.rules.filter(r=>ruleKeys(r).includes(f.key));return `<tr><td><b>${esc(f.name)}</b></td><td>${esc(f.type)}／${esc(f.unit||'—')}</td><td>${esc(f.source)}</td><td>${used.length?used.map(r=>`<div>${esc(r.name)} <small>（${r.enabled?'已啟用':'已停用'} · ${r.weight}%）</small></div>`).join(''):'<small>尚無規則使用，不計分</small>'}</td><td><button class="link" onclick="editField(${i})">編輯欄位</button></td></tr>`;}).join('')}</tbody></table></div><div class="pad notice">新增自訂欄位可用於匯入對應、工作台與 Excel 匯出。預期來源只是描述，不會自動抓取。</div></div>`;
  editField=i=>{const f=i==null?{name:'',type:'數字',unit:'',source:'',show:true}:db.fields[i];const builtIn=f.key&&!f.key.startsWith('custom_');openModal(i==null?'新增資料欄位':'編輯資料欄位',`<form onsubmit="submitField(event,${i??'null'})"><div id="form-error" class="form-error" role="alert"></div><div class="form-grid"><label class="field full">欄位名稱<input name="name" required maxlength="40" value="${esc(f.name)}" ${builtIn?'readonly':''}></label><label class="field">欄位類型<select name="type" ${builtIn?'disabled':''}>${options(['數字','金額','文字','日期','是／否'],f.type)}</select></label><label class="field">單位<input name="unit" maxlength="12" value="${esc(f.unit)}"></label><label class="field full">預期資料來源<input name="source" maxlength="100" value="${esc(f.source)}" placeholder="例如：園所提供的年度資料"></label></div><div class="notice">${builtIn?'內建欄位名稱與型別固定，以維持來源對應與計分一致。':'新增欄位不會自動納入風險計算，需另建立對應的可執行規則。'}</div><div class="form-actions"><button type="button" onclick="closeModal()">取消</button><button class="primary">儲存欄位</button></div></form>`);};
  submitField=(e,i)=>{e.preventDefault();const v=Object.fromEntries(new FormData(e.target)),old=i==null?null:db.fields[i],builtIn=old&&!old.key.startsWith('custom_'),name=builtIn?old.name:v.name.trim();if(!name||db.fields.some((f,j)=>j!==i&&f.name===name)){ $('#form-error').textContent='請填寫不重複的欄位名稱。';return;}const f={...old,key:old?.key||'custom_'+uid().replaceAll('-',''),name,type:builtIn?old.type:v.type,unit:v.unit.trim(),source:v.source.trim(),show:true};if(i==null)db.fields.push(f);else db.fields[i]=f;if(!persist()){if(i==null)db.fields.pop();else db.fields[i]=old;$('#form-error').textContent='無法保存欄位，請檢查本機儲存空間。';return;}const pos=numericImportFields.indexOf(f.key);if(['數字','金額'].includes(f.type)){if(pos<0)numericImportFields.push(f.key);}else if(pos>=0)numericImportFields.splice(pos,1);closeModal();settings();toast('已儲存欄位');};
