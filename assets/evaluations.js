...
(() => {
  ...
  Y.evaluations=(id,year=115)=>{currentSchool=Number(id);currentYear=Number(year);draw();};
  function draw(){
    ...
    openModal(schoolById(currentSchool).name+' · 年度評鑑資料',`<div class="evaluation-toolbar"><label>評鑑年度 <select aria-label="評鑑年度" onchange="Y.evaluations(${currentSchool},this.value)">${years.map(y=>`<option value="${y}" ${y===currentYear?'selected':''}>${dateLabel(y)}</option>`).join('')}</select></label></div><p class="data-hint">預設顯示 115 年；此處評鑑年度可獨立切換，不會變更工作台的分析年度。</p><div class="evaluation-list">${rows.map(r=>`<article class="source-card"><h3>${esc(r.title)} <span class="data-status ${r.result==='不符合'||r.result==='待改善'?'conflict':''}">${esc(r.result)}</span></h3><p>${esc(r.date)} · ${r.demo?'合成示範文件':'本機上傳文件'}</p><small>${esc(r.fileName)}</small><div class="form-actions"><button onclick="Y.viewEvaluation(${currentSchool},'${r.id}')">查看</button><button onclick="Y.downloadEvaluation('${r.id}')">下載評鑑資料</button></div></article>`).join('')||'<div class="empty-inline">此年度尚無評鑑資料，可在下方上傳。</div>'}</div><details class="evaluation-upload" open><summary>上傳評鑑資料</summary><form onsubmit="Y.uploadEvaluation(event)"><div class="form-grid" style="margin-top:16px"><label class="field full">文件名稱<input name="title" required maxlength="100" placeholder="例如：年度基本評鑑檢核表"></label><label class="field">民國年度<input name="year" type="number" min="1" max="300" required value="${currentYear}"></label><label class="field">評鑑日期<input name="date" type="date" required min="${currentYear+1911}-01-01" max="${currentYear+1911}-12-31" value="${currentYear+1911}-01-01"></label><label class="field">評鑑結果<select name="result">${options(['符合','待改善','不符合','免檢核','待確認'],'待確認')}</select></label><label class="field full">評鑑文件（最多 10 MB）<input name="file" type="file" required accept=".pdf,.xlsx,.docx,.csv,.txt,.png,.jpg,.jpeg"></label></div><p class="data-hint">只保存於此瀏覽器，不會上傳 AWS。清除網站資料會移除文件。上傳結果保留為年度紀錄，不自動覆寫其他來源或改變風險分數。</p><div id="evaluation-error" class="form-error" role="alert"></div><div class="form-actions"><button class="primary" ${busy?'disabled':''}>${busy?'儲存中…':'上傳並保存'}</button></div></form></details>`);
  ...
  Y.downloadEvaluation=async id=>{const record=entries(currentSchool).find(r=>r.id===id);if(!record)return;try{const blob=record.demo?new Blob([record.text],{type:'text/plain;charset=utf-8'}):await storage('readonly',id);if(!blob)throw Error('找不到本機文件，可能已清除網站資料，請重新上傳。');downloadBlob(blob,record.fileName);}catch(e){toast('下載失敗：'+e.message);}};
